...
- **Apt 安裝**：從 ROS apt 套件庫安裝 `realsense2_camera` 和 `librealsense2`
...
# 2. 執行（預設：roslaunch realsense2_camera rs_camera.launch）
./run.sh
...
| `IMAGE_NAME` | 映像檔名稱 | `realsense_noetic` |
...
| `runtime` | `ros:noetic-ros-base-focal` | RealSense 套件 + udev 規則 |
| `test` | `runtime` | Lint + smoke tests，建置後丟棄 |

## Smoke Tests

位於 `test/smoke_test/` — 在 `docker build --target test` 時自動執行 — 共 **23 項測試**。
...
| `realsense2_camera` | 可透過 `rospack find` 查詢到 |
| `realsense2_description` | 可透過 `rospack find` 查詢到 |
| `librealsense2` | 函式庫已安裝 |
| udev 規則 | `/etc/udev/rules.d/99-realsense-libusb.rules` 存在 |
