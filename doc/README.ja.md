...
`test/smoke_test/` に配置 — `docker build --target test` 時に自動実行 — 合計 **23 テスト**。
...
| `realsense2_camera` | `rospack find` で確認可能 |
| `realsense2_description` | `rospack find` で確認可能 |
| `librealsense2` | ライブラリがインストール済み |
| udev ルール | `/etc/udev/rules.d/99-realsense-libusb.rules` が存在する |
